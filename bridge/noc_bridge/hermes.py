    ...
    *,
    ...
    if task == "daily_report":
        ...
        task_rules = (
            ...
            "pattern_families as grouping candidates, then use log_excerpt and "
            "representative_entries for semantic context. Treat several physical "
            "templates from the same dependency, exception chain, and operation "
            "as one operational finding when the evidence supports the same "
            "failure cause. Do not create one finding per physical template or "
            "per logger layer. Prefer one finding with multiple pattern_ids and "
            "keep key_finds to the major causes and secondary_finds to genuinely "
            "independent lower-volume causes. "
