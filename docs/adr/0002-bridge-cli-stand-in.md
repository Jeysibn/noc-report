# ADR 0002: Milestone 12 Claude Bridge — keep the sandbox CLI call a stand-in
...
Milestone 12 builds the real Claude Bridge: RabbitMQ consumer, MinIO
...
real, credentialed `claude` CLI invocation inside the sandbox.

Wiring that now would mean mounting the operator's own live Claude Code
credentials (`~/.claude/.credentials.json` — the only credential available
...
("Start Claude Code CLI") through item 12 ("Capture structured output")
...
- Wiring the real `claude` CLI call (and the credential-scoping design
  ...
  RabbitMQ -> Bridge -> Docker -> Claude Code -> MinIO/Postgres -> UI).
