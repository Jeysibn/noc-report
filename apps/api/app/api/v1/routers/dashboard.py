...
def dashboard_summary(
    ...
    current_analysis = select(AnalysisRun.id).where(
    ...
    logs_awaiting = db.scalar(
        select(func.count()).select_from(Incident).where(shift_filter, active_log, ~current_analysis)
    ) or 0
    analyses_running = db.scalar(
        select(func.count()).select_from(Incident).where(shift_filter, running_analysis)
    ) or 0
    reports = db.scalar(
        select(func.count()).select_from(Report).where(Report.shift_id == shift_id)
    ) or 0
